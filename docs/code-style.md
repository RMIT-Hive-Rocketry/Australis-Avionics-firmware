...
- When naming functions, use underscore for spacing, and regular English case.
  For instance, `CAN_Transmit` is better than `CAN_transmit` or `CANTransmit`
...
- Make variable names descriptive. Names like `modX` or `d_ab` are terrible and
  extend debugging time. Don't be afraid of longer variable names. The one
  exception is the use of `i` in very short `for` loops, which is a matter of
  convention.
- Local variables should use lowercase, while globals should be named similarly
...
- Avoid comments that just say the code. None of this 'Set variable to be equal
  to 5' or 'Set GPIO pin to high' junk! If your comment takes longer to read
  than the code, it's pointless.
...
- Avoid comments at the end of code lines.
